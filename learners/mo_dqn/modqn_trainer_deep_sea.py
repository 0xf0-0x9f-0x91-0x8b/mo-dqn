...
class MODQNTrainer:
    ...
    def train(self, num_episodes: int, warmup_episodes: int = 1000,
        ...
        for _ in range(warmup_episodes):
            rollout_stats = self.replay_buffer.collect_rollout(self.env, self.policy, num_steps=100, epsilon=1.0)
        print('Buffer filled')
        print("-" * 60)
        ...
        for episode in range(num_episodes):
            ...
            # Collect rollout
            rollout_stats = self.replay_buffer.collect_rollout(self.env, self.policy, num_steps=200, epsilon=epsilon)
...
if __name__ == "__main__":
    ...
    config = '"deep-sea-treasure' # Or 'mones'
    num_episodes = 20000
    ...
    if config == '"deep-sea-treasure':
        num_actions, num_obs, num_objectives = 4, 2, 2
        env = mo_gym.make("deep-sea-treasure-v0") # https://mo-gymnasium.farama.org/environments/resource-gathering/
    # elif config == 'mones':
